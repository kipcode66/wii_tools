...
def patch_file(data: bytes, fileNumber: int, version: str, rel_name: str = "pcrel.bin"):
    data = bytearray(data)
    ...
    patchFilesBytes(
        0x1E4 + len(BIN_DATA_INIT[version]), BIN_DATA_MAIN[version] + bytes(rel_name, 'utf-8') + b'\x00')
    ...
    if args.command in ["inject", "generate"]:
        ...
        if not zeldaTp_idx is None:
            for file_idx in args.index:
                save_bin.files[zeldaTp_idx].data = bytes(
                    patch_file(save_bin.files[zeldaTp_idx].data, file_idx, args.game_version))
        else:
            if verbosity > 0:
        ...
        for file_idx in args.index:
            zeldaTp_data = bytes(patch_file(
                zeldaTp_data, file_idx, args.game_version, args.name))
